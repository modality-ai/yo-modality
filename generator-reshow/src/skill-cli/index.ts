 ...
 * skill-cli Generator
 *
 * Scaffolds a `modality-cli-kit` command-line toolkit — the same shape as the
 * `use-sentry` project: a `runner/cli.ts` entry, a `cli-core` runner config, a
 * command registry, and example commands. Package.json is composed from
 * `skill-cli-package-json`.
 */
...
export default class extends YoGenerator {
  ...
  async prompting() {
    ...
    handleAnswers(await mergePromptOrOption(prompts), (payload: any) => {
      console.log({ payload });
      composeWithBefore(require.resolve("../skill-cli-package-json"), payload);
    });
  }
